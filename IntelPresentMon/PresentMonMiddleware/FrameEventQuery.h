...
namespace pmon::mid
{
	...
	public:
		// functions
		PM_FRAME_QUERY(std::span<PM_QUERY_ELEMENT> queryElements, ipc::MiddlewareComms& comms);
		~PM_FRAME_QUERY();
		void GatherToBlob(uint8_t* pBlobBytes, const util::metrics::FrameMetrics& frameMetrics) const;
	...
	private:
		...
		static void GatherFromFrameMetrics_(const GatherCommand_& cmd, uint8_t* pBlobBytes, const util::metrics::FrameMetrics& frameMetrics);
		...
		// data
		const ipc::MiddlewareComms& comms_;
