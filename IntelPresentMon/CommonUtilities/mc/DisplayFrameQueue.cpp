...
namespace pmon::util::metrics
{
        ...
        SwapChainCoreState& chain)
    {
        ...
        for (size_t displayIndex = 0; displayIndex < displayCount; ++displayIndex) {
            ...
            CompleteOpenTimelineDisplayLookahead_(row.screenTime);
    ...
    void DisplayFrameQueue::CompleteOpenTimelineDisplayLookahead_(uint64_t nextScreenTime)
    {
        for (auto pending = openTimelineRows_.rbegin(); pending != openTimelineRows_.rend(); ++pending) {
            if (pending->row.isDisplayed) {
                if (!pending->displayTimingComplete) {
                    pending->row.nextScreenTime = nextScreenTime;
                    pending->displayTimingComplete = true;
                }
                return;
            }
        }
        ...
        std::vector<ReadyDisplayRow>& ready)
    {
        ...
        closedInterval.reserve(openTimelineRows_.size() + 1);
        for (auto& pending : openTimelineRows_) {
        ...
        std::vector<ReadyDisplayRow>& ready)
    {
        ...
        for (size_t i = 0; i < intervalRows.size(); ++i) {
            if (intervalRows[i].row.isDisplayed) {
                intervalRows[i].row.animation = contexts[animationIndex++];
            }
    ...
    void DisplayFrameQueue::PublishSourceTransition_(
        ...
        std::vector<ReadyDisplayRow>& ready)
    {
        CompleteOpenTimelineDisplayLookahead_(transitionAppRow.screenTime);
        ...
        ReleaseAll_(openTimelineRows_, ready);
