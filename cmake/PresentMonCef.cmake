...
# The CEF stage holds wrapper libraries compiled by the selected platform
# toolset, so it is qualified as <dependency-root>/cef/<toolset> and shared only
# by build trees using that toolset. The archive cache and the restore lock
# below it stay shared. The stage remains separate from the MSBuild stage in
...
function(pmon_configure_cef)
    ...
    set(cef_root "${PMON_DEPENDENCY_ROOT}/cef/${PMON_VS_TOOLSET}")
    ...
    if(PMON_CEF_SOURCE)
        set(restore_source_argument --source "${PMON_CEF_SOURCE}")
    endif()

    # This explicit target can repair or refresh the automatically prepared
    # stage without recreating the CMake build tree.
    add_custom_target(
        pmon_restore_cef
        COMMAND
            "${PMON_PYTHON_EXECUTABLE}"
            "${PROJECT_SOURCE_DIR}/Scripts/cef.py"
            restore
    ...
    add_custom_target(
        ...
        COMMAND
            ...
            --repo-root "${PROJECT_SOURCE_DIR}"
            --dependency-root "${PMON_DEPENDENCY_ROOT}"
            --output-root "${runtime_destination}"
